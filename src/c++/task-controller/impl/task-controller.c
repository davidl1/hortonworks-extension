...
#include <dirent.h>
#include <fcntl.h>
// dlai added 12/29/2012
#include <libgen.h>
#ifdef linux
#include <fts.h>
#else
#define _XOPEN_SORUCE 500
#include <stdio.h>
#include <ftw.h>
#include <unistd.h>
#endif
...
// dlai added (int) 12/29/2012
            " the group of the executable %d\n", (int) getgid(), binary_gid);
    return -1;
  }
...
//  dlai blocked 12/29/2012  if (mkdirat(cwd, token, perm) != 0) {
      if (fchdir(cwd) < 0 || mkdir(token, perm) != 0) {
      if (errno != EEXIST) {
 ...
 */ 
signal_user_task(const char *user, int pid, int sig) {
  if(pid <= 0) {
 ...
 */
// added modified 12/29/2012
#ifdef linux
static int delete_path(const char *full_path, 
                       int needs_tt_user) {
  int exit_code = 0;
  
// dlai blocked all 12/29/2012
/* ===================================================================================
  if (full_path == NULL) {
...
static int delete_path(const char *full_path,
  ...
  if (full_path == NULL) {
    fprintf(LOGFILE, "Path is null\n");
    return UNABLE_TO_BUILD_PATH; // may be malloc failed
  } else {
    int i = 0;
    char *(paths[]) = {strdup(full_path), 0};
    if (access(full_path, F_OK) != 0) {
    ...
    while (paths[i] != NULL) {
      while (paths[i][strlen(paths[i]) - 1] == '/')
        paths[i][strlen(paths[i]) -1] = '\0';
      if ((rc = nftw(paths[i], unlink_cb, 64, FTW_DEPTH | FTW_PHYS)) != 0) {
        if ((rc = nftw(paths[i], unlink_cb, 64, FTW_DEPTH | FTW_PHYS)) != 0) {
          if (rc) {
            fprintf(LOGFILE, "Error traversing directory %s [%d] - %s\n",
                    paths[i], rc, strerror(errno));
            break;
          }
        }
      }
    ...
    if (needs_tt_user) {
      //rc = rmdir_as_nm(full_path);
      rc = rmdir_as_tasktracker(full_path);
    }
    free(paths[0]);
    return rc;
  }
