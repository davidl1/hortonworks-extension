...
static int is_only_root_writable(const char *file) {
  ...
  if ((file_stat.st_mode & (S_IWGRP | S_IWOTH)) != 0) {
    fprintf(LOGFILE, 
	    // dlai blocked 12/28/2012 "File %s must not be world or group writable, but is %03o\n",
...
void read_config(const char* file_name) {
  ...
  while(!feof(conf_file)) {
    ...
    // size_read = dlai added getline(&line,&linesize,conf_file);
    size_read = (int) getline(&line,&linesize,conf_file);
